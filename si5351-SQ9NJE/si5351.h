...
#define PLL_A			26
#define PLL_B			34
#define PLL_REG_A		0x16
#define PLL_REG_B		0x17
#define PLL_MULT		36	//900 MHz when switching from FRAC_PLL
...
#define FREQ_DIV4		150000000	//>150 MHz need DIVBY4
#define FREQ_DIV6		112500000	//>112.5 MHz need DIVBY6
...
class si5351
{
	...
	public:
		...
		void pll_config(uint8_t pll, uint8_t multiplier, 
		...
		void set_frequency_FIXDIV(uint8_t clk, uint32_t freq, uint8_t DIV);
		void simple_set_frequency(uint8_t clk, uint32_t freq);
		void get_params(uint32_t &a, uint32_t &b, uint32_t &c, uint8_t &frac);
		void calc_divider(uint8_t mul, uint32_t tgt, uint32_t ref);
		void set_config(uint8_t reg, uint32_t mul, uint32_t num, uint32_t denom);
};
